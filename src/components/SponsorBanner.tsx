...
export function SponsorBanner({ sectionKey, categoryId = null }: SponsorBannerProps) {
  ...
  const activeSponsor = category?.sponsor_url
  ...
  const className = 'mx-4 rounded-2xl overflow-hidden border border-gray-800 bg-black block'
  const style = { height: 110 }

  if (category?.sponsor_link_url) {
    return (
      <a key={sectionKey} href={category.sponsor_link_url} target="_blank" rel="noopener noreferrer" className={className} style={style}>
        {image}
      </a>
    )
  }
  ...
  return (
    <div key={sectionKey} className={className} style={style}>
      {image}
    </div>
  )
}
